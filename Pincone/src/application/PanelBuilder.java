	...
	/*
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if ( result == JOptionPane.OK_OPTION ) {
					try {
						WareHouse currentWarehouse = warehouses.get( new Integer((int)txtStoreId.getValue()) );
						
						currentWarehouse.updateFullStock( currentWarehouse.getProductBySKU(Integer.parseInt(sku.getText())), Integer.parseInt(fullyStocked.getText()) );
		...
		button2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (result == JOptionPane.OK_OPTION) 
				{
					...
					Cart c = new Cart("W0-S" + currentWarehouse.getwhID() );		// TODO: add way for HQ store change
					for( Product p1 : Store.getStoreByID(currentWarehouse.getwhID()).getProducts() )	// gets Store by static Store list
				    {
						if( p1.getStockPar() - p1.getStockIn() > 0 )
						{
							// if Product needs to be re-stocked  |  AUTO-ORDER amount is > 0 
				    ...
				    String reciept = "ORDER_ID : W0-S" + currentWarehouse.getwhID() +"\n"; // TODO: add wh to each STORE or hardcode to W0
					String[][] cart = c.cartToPrint();				// [ SKU ][ NAME ][ QUANTITY ][ @ AMOUNT ][ LINE TOTAL ]
					for( String[] line : cart )
					{
						// Compiles cart to receipt
